...
def student():
    ...
    return render_template('admin/student/index.html', rows=rows)
...
@app.route('/admin/edit_student_name/<string:student_firstname>/<string:student_lastname>')
def edit_student_name(student_firstname, student_lastname):
    con = sql.connect("student_ss34.db")
    con.row_factory = sql.Row
    cur = con.cursor()
    cur.execute(f"select * from students where firstName = '{student_firstname}' and lastName = '{student_lastname}'")
    rows = cur.fetchall()
...
@app.route('/admin/detail_student_name/<string:student_firstname>/<string:student_lastname>')
def detail_student_name(student_firstname, student_lastname):
    con = sql.connect("student_ss34.db")
    con.row_factory = sql.Row
    cur = con.cursor()
    cur.execute(f"select * from students where firstName = '{student_firstname}' and lastName = '{student_lastname}'")
